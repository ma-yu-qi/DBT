...
def predict(model, config, logger):
    ...
    for i, item in enumerate(dataset):
        item = znorm(item)
        grid_sampler = tio.inference.GridSampler(item, patch_size=(config.patch_size), patch_overlap=(28, 28, 24))
        ...
        with torch.no_grad():
            for j, batch in enumerate(patch_loader):
                ...
                if config.network == "dbt":
                    ...
                    # 在预测中我们选择 pred_2 作为最终输出
                    pred = pred_2
                    mask = pred_2.argmax(dim=1, keepdim=True)

                elif config.network == "IS" or config.network == "dbt_base":
                    # 获取 IS 类网络的两个输出
                    pred_1, pred = model(x)
...
def main(config):
    ...
    elif config.network == "fcn3d":
        from models.three_d.fcn3d import FCN_Net

        model = FCN_Net(in_channels=config.in_classes, n_class=config.out_classes)
    ...
    elif config.network == "fusionnet":
        from models.three_d.FusionNet import FusionNet

        model = FusionNet(in_channels=config.in_classes, out_channels=config.out_classes)
